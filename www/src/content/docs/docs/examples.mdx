...
:::note
If you have a typescript project, `npx sst deploy` might fail with type errors during build time, to fix it avoid type checking during deploy
:::

```json title="package.json" {3}
"scripts": {
    "build": "tsc -b && vite build",
    "build-sst" : "vite build", 
    "dev": "vite",
    "lint": "eslint .",
    "preview": "vite preview"
  },
```
Now, change the build command in `sst.config.ts` or just use `path` to `dist` folder 

```ts title="sst.config.ts"
new sst.aws.StaticSite("Web", {
  build: {
    command: "pnpm run build-sst",
    output: "dist",
  },
});
```
